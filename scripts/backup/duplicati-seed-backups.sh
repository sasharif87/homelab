#!/bin/bash
# Create Duplicati backup jobs via API
# Requires DUPLICATI_WEBPASSWORD and DUPLICATI_BACKUP_PASSPHRASE in the environment
# (see Docs/secrets.md)

DUPLICATI="http://localhost:8200"
WEBPASSWORD="${DUPLICATI_WEBPASSWORD:?Set DUPLICATI_WEBPASSWORD}"
PASSPHRASE="${DUPLICATI_BACKUP_PASSPHRASE:?Set DUPLICATI_BACKUP_PASSPHRASE}"

# Get access token
TOKEN=$(curl -s -X POST "$DUPLICATI/api/v1/auth/login" \
  -H "Content-Type: application/json" \
  -d "{\"Password\":\"$WEBPASSWORD\"}" | python3 -c "import sys,json; print(json.load(sys.stdin)['AccessToken'])")

if [ -z "$TOKEN" ]; then
  echo "Failed to get auth token"
  exit 1
fi

AUTH="Authorization: Bearer $TOKEN"

create_backup() {
  local name="$1" target="$2" sources_json="$3" hour="$4"
  echo "Creating: $name"
  result=$(curl -s -X POST "$DUPLICATI/api/v1/backups" \
    -H "$AUTH" \
    -H "Content-Type: application/json" \
    -d "{
      \"Backup\": {
        \"Name\": \"$name\",
        \"TargetURL\": \"file://$target\",
        \"DBPath\": \"@\",
        \"Sources\": $sources_json,
        \"Settings\": [
          {\"Name\": \"encryption-module\", \"Value\": \"aes\"},
          {\"Name\": \"compression-module\", \"Value\": \"zip\"},
          {\"Name\": \"passphrase\", \"Value\": \"$PASSPHRASE\"},
          {\"Name\": \"retention-policy\", \"Value\": \"1W:1D,4W:1W,12M:1M\"}
        ],
        \"Filters\": [],
        \"Metadata\": {}
      },
      \"Schedule\": {
        \"Time\": \"2026-01-01T0${hour}:00:00Z\",
        \"Repeat\": \"1D\",
        \"AllowedDays\": null,
        \"Tags\": []
      }
    }")
  echo "$result" | python3 -c "import sys,json; d=json.load(sys.stdin); print('  ✓ Created, ID:', d.get('ID', d))" 2>/dev/null || echo "  Response: $result"
}

# jellyfin-config lives on the docker-local NVMe zvol (mounted in compose/duplicati.yml).
# It MUST be listed explicitly — it is not under /source/appdata. See the June 19 2026 wipe.
create_backup "Appdata Daily" "/backups/appdata" '[ "/source/appdata", "/source/jellyfin-config" ]' "3"
create_backup "Config Daily"  "/backups/config"  '[ "/source/compose", "/source/npm" ]' "3:15"

echo ""
echo "Done. Passphrase used was read from DUPLICATI_BACKUP_PASSPHRASE — confirm it matches Docs/secrets.md / Vaultwarden."
